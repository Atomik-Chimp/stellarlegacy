...
dnl -----> Modifications for SL
dnl AC_CXX_NAMESPACES
dnl Version 1.2 (1999/10/11)
dnl written by Luc Maisonobe
...
dnl Version 1.0
dnl adapted from AC_CXX_HAVE_STD

AC_DEFUN(AC_CXX_HAVE_SSTREAM,
[AC_CACHE_CHECK(whether the compiler supports sstream,
...
AC_DEFINE(HAVE_SSTREAM,,[define if the compiler supports sstream])
fi
])



AC_DEFUN(AC_CXX_FINDSTD,
[AC_MSG_CHECKING([where libstdc++ v3 is located])
libstdcxxdir=`locate libstdc++.so.3.0.0 | sed 'q' |sed 's/\(.*\)\/\(lib\)\/\(libstdc++.so.3.0.0\)\(.*\)/\1/'`
 if test "$libstdcxxdir" = "" ; then
  AC_MSG_RESULT(not found)
  libstdcxxdir = ""
  if test "$ac_cv_cxx_have_sstream" = no; then
   echo "*** neither libstdc++3 nor sstream could be      ***"
   echo "*** found on your system. If you are sure that   ***"
   echo "*** you have installed libstdc++ try running     ***"
   echo "*** updatedb                                     ***"
   echo "*** everything that needs libdstdc++ (logging)   ***"
   echo "*** will be disabled in this build               ***"
  fi
 else
  AC_MSG_RESULT(found)
  AC_DEFINE(HAVE_LIBSTDCXX,,[define if libstdc++v3 was found])                   fi
])
